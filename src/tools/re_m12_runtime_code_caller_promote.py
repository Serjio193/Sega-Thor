"""Promote one caller-backed runtime-observed exact 68000 routine."""
import argparse
import copy
import hashlib
...
START, END = 0x03B358, 0x03B486
CALLERS = (0x03A900, 0x03AA2A)
    ...
    for index, entry in enumerate(entries):
        ...
        if entry["start"] <= START and END <= entry["end"]:
            ...
            replacement.append({
                ...
                "source": "M12_AUTO42_runtime_caller_backed_exact_routine",
                "confidence": "CONFIRMED",
                "classification": "CALLER_BACKED_RUNTIME_EXACT_ASM_ROUNDTRIP",
                "emitted_artifact_type": "asm",
                "ownership_reason": (
                    "two exact static BSR callers, complete runtime-observed decoded "
                    "range, and canonical-ROM-equal ASM with RTS boundary"),
            })
            if END < entry["end"]:
...
def validate_evidence(root, rom, runtime):
    ...
    if not all(path.exists() for path in (asm, binary, detail)):
        raise ValueError("missing AUTO42 exact routine evidence")
    if binary.read_bytes() != rom[START:END]:
        raise ValueError("AUTO42 binary differs from canonical ROM")
    contract = json.loads(detail.read_text())
    if contract != {"start": START, "end": END, "terminator": "RTS", "roundtrip": "EXACT"}:
        raise ValueError("AUTO42 decoder/round-trip contract changed")
    observed = {int(address, 16) for address in runtime["executed_addresses"]}
    if START not in observed:
        raise ValueError("AUTO42 entry PC was not runtime-observed")
    facts = [item for item in runtime["execution_facts"]
             if START <= int(item["address"], 16) < END]
    if len(facts) != 73 or any(item.get("decoder_status") != "DECODED" for item in facts):
        raise ValueError("AUTO42 runtime facts are not the complete decoded routine")
    for caller in CALLERS:
        expected = bytes.fromhex("6100")
        if rom[caller:caller + 2] != expected:
            raise ValueError(f"AUTO42 caller contract changed at 0x{caller:06X}")
    return {"start": START, "end": END, "bytes": END - START,
            "callers": list(CALLERS), "runtime_observed_instruction_starts": len(facts),
...
def run(args):
    ...
    manifest["transaction"] = "M12-AUTO42 runtime caller-backed exact routine"
    (materialized / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    rebuilt = (materialized / "rebuilt.rom").read_bytes()
    report = {"schema": "oasis.m68k.m12-auto42-runtime-code.v1", "routine": evidence,
