"""Promote the exact six fixed-width menu labels at 0x05CBA6."""
import argparse
import copy
import hashlib
...
TABLE_START = 0x05CBA6
TABLE_END = 0x05CBD6
RECORD_SIZE = 8
RECORD_COUNT = 6
TABLE_BYTES = bytes.fromhex(
    "06574541504F4E20054954454D5320200653544154555320034D415020202020"
    "0453415645202020034F4B3F20202020")
    ...
    for index, entry in enumerate(entries):
        ...
        if entry["start"] <= TABLE_START and TABLE_END <= entry["end"]:
            ...
            replacement.append({
                ...
                "source": "M12_AUTO32_label_table",
                "confidence": "CONFIRMED",
                "classification": "FIXED_WIDTH_MENU_LABEL_TABLE",
                "emitted_artifact_type": "rom_asset",
                "ownership_reason": "0x003F8E selects six exact 8-byte records",
            })
        ...
        if TABLE_START < entry["end"] and entry["start"] < TABLE_END:
            raise ValueError("label table overlaps existing ownership")
    raise ValueError("label table is not wholly UNKNOWN")
...
def parse_contract(rom):
    ...
    if actual != TABLE_BYTES or len(actual) != RECORD_SIZE * RECORD_COUNT:
        raise ValueError("label table bytes changed")
    return {"start": TABLE_START, "end": TABLE_END, "bytes": len(actual),
            "record_size": RECORD_SIZE, "record_count": RECORD_COUNT,
            "consumer_address": 0x003F8E,
            "selector_source": "FF185C -> FF1861; switch 1..5 or explicit 5",
            "reason": "FF1861 << 3; LEA 0x05CBA6; fixed 8-byte label records"}
...
def run(args):
    ...
    manifest["transaction"] = "M12-AUTO32 exact menu label table"
    (materialized / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    rebuilt = (materialized / "rebuilt.rom").read_bytes()
    report = {"schema": "oasis.m68k.m12-auto32-label-table-promotion.v1",
