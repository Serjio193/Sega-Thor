"""Promote one bounded runtime-observed exact 68000 routine range."""
import argparse
import copy
import hashlib
    ...
    for index, entry in enumerate(entries):
        ...
        if entry["start"] <= START and END <= entry["end"]:
            ...
            replacement.append({
                ...
                "source": "M12_AUTO41_runtime_observed_exact_routine",
                "confidence": "CONFIRMED",
                "classification": "RUNTIME_OBSERVED_EXACT_ASM_ROUNDTRIP",
                "emitted_artifact_type": "asm",
                "ownership_reason": (
                    "runtime-observed entry, complete decoded instruction stream, "
                    "explicit RTS boundary, and canonical-ROM-equal ASM"),
            })
            if END < entry["end"]:
...
def validate_evidence(root, rom, runtime):
    ...
    if not all(path.exists() for path in (asm, binary, detail)):
        raise ValueError("missing AUTO41 exact routine evidence")
    if binary.read_bytes() != rom[START:END]:
        raise ValueError("AUTO41 binary differs from canonical ROM")
    contract = json.loads(detail.read_text())
    if contract != {"start": START, "end": END, "terminator": "RTS", "roundtrip": "EXACT"}:
        raise ValueError("AUTO41 decoder/round-trip contract changed")
    observed = {int(address, 16) for address in runtime["executed_addresses"]}
    if START not in observed:
        raise ValueError("AUTO41 entry PC was not runtime-observed")
    facts = [item for item in runtime["execution_facts"]
             if START <= int(item["address"], 16) < END]
    if len(facts) != 100 or any(item.get("decoder_status") != "DECODED" for item in facts):
        raise ValueError("AUTO41 runtime facts are not the complete decoded routine")
...
def run(args):
    ...
    manifest["transaction"] = "M12-AUTO41 runtime-observed exact routine"
    (materialized / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    rebuilt = (materialized / "rebuilt.rom").read_bytes()
    report = {"schema": "oasis.m68k.m12-auto41-runtime-code.v1", "routine": evidence,
