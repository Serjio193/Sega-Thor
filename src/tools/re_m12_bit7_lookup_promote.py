"""Promote the exact 64-byte bit-7 lookup at 0x05CE16."""
import argparse
import copy
import hashlib
...
TABLE_SHA256 = "1aa8fef8c9221c3ff8fc55bc8329944fd7588b60f763294c4642d902a657eca3"
TABLE_START = 0x05CE16
TABLE_END = 0x05CE56
CONSUMER = 0x00F61C
INDEX_MASK = 0x3F
    ...
    for index, entry in enumerate(entries):
        ...
        if entry["start"] <= TABLE_START and TABLE_END <= entry["end"]:
            ...
            replacement.append({
                ...
                "source": "M12_AUTO35_bit7_lookup",
                "confidence": "CONFIRMED",
                "classification": "BYTE_BIT7_LOOKUP_TABLE",
                "emitted_artifact_type": "rom_asset",
                "ownership_reason": "0x00F61C masks the index to 0x3F and tests bit 7",
            })
        ...
        if TABLE_START < entry["end"] and entry["start"] < TABLE_END:
            raise ValueError("bit-7 lookup overlaps existing ownership")
    raise ValueError("bit-7 lookup is not wholly UNKNOWN")
...
def parse_contract(rom):
    ...
    if len(actual) != TABLE_END - TABLE_START or hashlib.sha256(actual).hexdigest() != TABLE_SHA256:
        raise ValueError("bit-7 lookup bytes changed")
    return {"start": TABLE_START, "end": TABLE_END, "bytes": len(actual),
            "consumer_address": CONSUMER, "index_mask": INDEX_MASK,
            "reason": "MOVE.W 50(A6),D0; ANDI.W #$3F,D0; BTST.B #7,(A2,D0.W)"}
...
def run(args):
    ...
    manifest["transaction"] = "M12-AUTO35 exact bit-7 lookup"
    (materialized / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    rebuilt = (materialized / "rebuilt.rom").read_bytes()
    report = {"schema": "oasis.m68k.m12-auto35-bit7-lookup-promotion.v1",
