...
std::uint64_t job_hash(const FrontierRecord& frontier, std::string_view rom_sha256) {
    ...
    const auto values = std::array<std::string, 8>{frontier.id, std::string(rom_sha256),
        std::to_string(frontier.source_entry), std::to_string(frontier.source_pc), frontier.known_context,
        "RESOLVE_INDIRECT_TARGET", "natural_reset_idle_v1", "neutral_only"};
...
AntJob make_ant_job(const FrontierRecord& frontier, std::string rom_sha256,
    ...
    AntJob result{.job_id = "ant-" + hash_hex(job_hash(frontier, rom_sha256)), .frontier_id = frontier.id,
                  .rom_sha256 = std::move(rom_sha256), .rom_size = rom_size, .backend_version = std::move(backend_version),
                  .scenario_id = "natural_reset_idle_v1", .source_entry = frontier.source_entry,
                  ...
                  .known_static_context = frontier.known_context, .max_steps = 3000000, .max_frames = 300};
    return result;
}
...
std::optional<FrontierRecord> select_ant_frontier(std::string_view explore_json, std::uint32_t source_pc) {
    ...
    if (frontier_start == std::string_view::npos) return std::nullopt;
    const auto object = source_object(explore_json.substr(frontier_start), source_pc);
