param([string]$RunName = ('v1dense-' + (Get-Date -Format 'yyyyMMdd-HHmmss')))
$ErrorActionPreference = 'Stop'
$root = (Resolve-Path (Join-Path $PSScriptRoot '../../../..')).Path
$harness = Join-Path $root 'build/bizhawk-controlled-harness/run.ps1'
$collector = Join-Path $PSScriptRoot 'dense.lua'
$normalizer = Join-Path $root 'src/tools/thor_evidence/normalize.py'
$receipt_tool = Join-Path $root 'src/tools/thor_evidence/receipt.py'
...
$raw = Join-Path $root ('build/thor-evidence/v1-gate-coverage/' + $RunName + '.raw.jsonl')
$receipt = Join-Path $root ('build/thor-evidence/v1-gate-coverage/' + $RunName + '.receipt.json')
$sealed = Join-Path $root ('build/thor-evidence/v1-gate-coverage/' + $RunName + '.sealed.jsonl')
