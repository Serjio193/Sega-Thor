"""Promote the exact 16-entry nibble lookup at 0x062DC0."""
import argparse
import copy
import hashlib
...
TABLE_START = 0x062DC0
TABLE_END = 0x062DE0
VALUES = (0x0D5C, 0x0C9C, 0x0BE8, 0x0B3C, 0x0A9C, 0x0A04, 0x0974, 0x08EC,
          0x086C, 0x07F0, 0x0780, 0x0714, 0xFFFF, 0xFFFF, 0x00FF, 0xFFFF)
    ...
    for index, entry in enumerate(entries):
        ...
        if entry["start"] <= TABLE_START and TABLE_END <= entry["end"]:
            ...
            replacement.append({
                ...
                "source": "M12_AUTO28_nibble_lookup",
                "confidence": "CONFIRMED",
                "classification": "PC_RELATIVE_NIBBLE_LOOKUP_TABLE",
                "emitted_artifact_type": "rom_asset",
                "ownership_reason": "0x0624D2 forms the base; D3 is masked with "
                "0xF, doubled, and used for 16 word reads",
            })
        ...
        if TABLE_START < entry["end"] and entry["start"] < TABLE_END:
            raise ValueError("nibble table overlaps existing ownership")
    raise ValueError("nibble table is not wholly UNKNOWN")
...
def parse_contract(rom):
    ...
    if values != VALUES:
        raise ValueError("nibble lookup bytes changed")
    return {"start": TABLE_START, "end": TABLE_END, "bytes": TABLE_END - TABLE_START,
            "entry_count": len(values), "values": values,
            "reason": "D3 masked with 0xF, doubled, and used by move.w 0(A2,D3.W)"}
...
def run(args):
    ...
    manifest["transaction"] = "M12-AUTO28 exact nibble lookup"
    (materialized / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    rebuilt = (materialized / "rebuilt.rom").read_bytes()
    report = {"schema": "oasis.m68k.m12-auto28-nibble-lookup-promotion.v1",
