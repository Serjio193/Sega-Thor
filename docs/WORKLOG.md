...
# 2026-09-13 — THOR Evidence Engine stabilization P2 — VERIFIED LOCALLY
...
# 2026-09-25 — M14.2B-PUBLISH2 MASTER V2 persistence restoration — PASS

**Objective:** Restore the omitted M14.2A MASTER V2 canonical read/materialize contract on the accepted fusion branch, prove complete logical persistence, and recover reproducible pre-push validation without changing fusion or SOURCE_OWNED.

**Audit and decision:** The accepted M14.2A `master_v2_shadow_test.py` imports `master_canonical_view`; its M14.2A worklog records both MASTER V2 and canonical-view tests passing. The missing read-layer module and its dedicated test have no tracked history but were present in the dirty original checkout and those exact tests pass there. This is an omitted accepted R2 dependency, not obsolete test code. Restored only those two files. Removed optional R3 resolver imports that depended on unrelated untracked modules, retained the accepted R2 pointer and configured-path behavior, and made materialization fail closed unless every canonical logical hash matches MASTER V2 metadata. Expanded the existing MASTER V2 fixture to prove nonempty ROM map, claims, relations, source/evidence, conflict, import, emission, derivation, and proposal rows survive serialization and reopened SQLite.

**Validation:** MASTER V2 and canonical-view tests PASS (3/3 each); Cartographer PASS (13), ROM-link PASS, M14.2A PASS (3), fusion unit PASS (3), and canonical knowledge pipeline PASS (18). Debug and Release builds PASS; full CTest PASS in both (209/209 each). M14.2B replay acceptance PASS with graph hash `2f612d1d8ea92e66897e587e1a70b303bd646ff59f3c520038596e96902b0a87`, order-independent graph, repeat-import idempotence and SQLite reopen persistence; emission-derived 2,489 ranges cover 3,145,728 bytes with zero gaps/overlaps, SOURCE_OWNED 1,487,672 and delta 0, emission hash `44a2332b0b433c635e33767886ffff35985ad31131e3dc3b4dea5e6984b17d92`. The authoritative SQLite database was opened read-only; `PRAGMA quick_check` returned `ok`, 2,489 emission rows were present, and source-owned emitted bytes totaled 1,487,672. Fresh worktree at the resulting commit passed MASTER V2 (3/3), canonical materialization (3/3), M14.2A (3/3), fusion unit (4/4), and two-capture fusion acceptance with graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`. Both materialization imports resolved inside that worktree; `git status` was clean.

**Cross-worktree reproducibility correction:** Fresh-worktree replay exposed a real identity mismatch: the graph hash differed because the derivation fingerprint used raw `rom_knowledge_fusion.py` bytes, and `core.autocrlf=true` produced different line endings between worktrees. Normalized only the implementation fingerprint to LF; source artifact hashes and all admitted facts remain byte-exact. Added a regression test for LF/CRLF equivalence. After normalization, forward and reverse replay both produce graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`. The amended source state passed the required fresh-worktree run: master_v2_shadow 3/3, master_canonical_view 3/3, M14.2A acceptance 3/3, fusion tests 4/4, and two-capture fusion acceptance. Imported `master_canonical_view.py` and `master_v2_shadow.py` both resolved inside the fresh worktree, and it remained clean. The M14.2B admission facts and map metrics remain the same: 2,489 ranges, zero gaps/overlaps, SOURCE_OWNED 1,487,672 (delta 0), and emission hash `44a2332b0b433c635e33767886ffff35985ad31131e3dc3b4dea5e6984b17d92`. Debug/Release builds PASS; Debug/Release full CTest PASS (209/209 each), with the final Debug run performed sequentially after an unrelated shared-output collision in a concurrent Debug/Release run.

# 2026-09-25 — M14.2C global graph to canonical map closure — PASS_GLOBAL_GRAPH_CANONICAL_MAP_CLOSURE_V1

**Objective:** Evaluate exact, fused graph facts against the canonical ROM map and produce deterministic parent-bound proposals, dry-run validation, and a child generation only where supported by exact proof. Preserve ROM identity, emission bytes, and SOURCE_OWNED; keep hypotheses and unresolved facts graph-only.

**Acceptance:** Load the accepted canonical generation and reproduce the M14.2B graph hash. Rank eligible and ineligible graph evidence; bind proposals to canonical generation/map/emission and graph hashes; validate exact operation preconditions and stale-parent rejection; report a full-ROM map delta or a fail-closed ranked UNKNOWN queue. Replay twice and compare proposal, partition, logical generation, ownership, and ranking identities. Run M14.2A/M14.2B, canonical/proposal/generation tests, Debug/Release builds and CTest, full-ROM and ownership checks, and a final clean-worktree smoke before publishing.

**Implementation:** Added a deterministic closure over the accepted fused graph. Two captures each prove the same exact RAM-shadow→DMA→hardware-SAT path through an existing canonical M68K instruction object. They merge into one stable `ADD_REFERENCE` claim keyed by canonical object and reference kind; its provenance keeps four capture-specific source artifacts, four exact relations, and both fusion derivations. The proposal binds the parent generation, map hash, emission hash and graph hash. A copied child SQLite generation validates the parent and each proof, persists the claim and evidence refs in the existing schema, and verifies the global object view exposes the canonical owner and both graph and projection evidence. No new boundary/classification/symbol is inferred; the proposal makes no `SOURCE_OWNED` change. Hypothesis, runtime-only and unresolved statuses remain ineligible without an exact fusion operation.

**Acceptance results:** Input graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`; exact graph facts 9,228; 4 exact relation facts admitted into 1 multi-source proposal across 2 captures; 1,007 hypotheses and 2,249 relation-level OBSERVED_RUNTIME facts are not directly map-admitted. The proposal validates; stale-parent validation rejects after child generation activation. Determinism replay matches proposal id/order, range partition, map hashes, child SQLite SHA-256 `9f199dd805f8a3af300367fec18824d015f440efa6f8fac0a31cece788c967d0`, and ranked UNKNOWN intervals. Partition remains 2,489 ranges, 3,145,728 bytes, zero gaps/overlaps; 768 UNKNOWN ranges / 1,658,056 bytes; ASM 56,418, DATA_TABLE metric 533,182, POINTER_TABLE metric 1,076, RESOURCE 1,085,110; each before/after metric is unchanged. Emission hash remains `44a2332b0b433c635e33767886ffff35985ad31131e3dc3b4dea5e6984b17d92`; SOURCE_OWNED remains 1,487,672 (delta 0). Canonical ROM identity is SHA-256 `eb19bda4982366a2fd43d65ab8a7f9709d83a8cc902c14a682c088c16359c263`, size 3,145,728. This is a reference-only map improvement; no new boundaries or classified bytes were proved.

**Validation:** M14.2A acceptance 3/3; M14.2B fusion acceptance PASS with order-independent graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`, 2,489 ranges, zero gaps/overlaps and SOURCE_OWNED delta 0. Canonical map 2/2, closure 3/3, fusion unit 4/4, pipeline 18/18, MASTER V2 3/3, canonical materialization 3/3. Debug build PASS and full CTest PASS (210/210); Release build PASS and full CTest PASS (210/210), including the stable-capture-identity regression. Fresh-worktree smoke at commit `21d6959394d5a05cf7baa1e1a09bc74cd3cf7d68` passed closure (3/3), M14.2A (3/3), fusion (4/4), regenerated one proposal over two captures from the accepted graph hash, and left the worktree clean. Project source limit passed (737 governed files ≤500 lines); `git diff --check` passes. No canonical input or ROM binary was modified; canonical SHA-256 identity is retained in map metadata and the exact full-size emission partition was audited.

# 2026-09-25 — M14.3 unknown ROM range closure campaign

**Objective:** Build a deterministic, map-first queue over every UNKNOWN canonical emission range, join existing global SQLite graph signals without treating ranking or hypotheses as truth, and run a bounded exact-proof screening pass. Preserve canonical ROM identity, emission partition, and SOURCE_OWNED. No game-specific PC rules and no PLAYER/entity semantics.

**Acceptance:** Reproduce the M14.2B graph and M14.2C canonical-map identities; account for all 768 UNKNOWN ranges / 1,658,056 bytes; report requested execution/reference/consumer/source/adjacency signals and exact blocker categories; rank deterministically; reject hypothesis-only admission; emit no map operation absent exact extent proof; keep 2,489 ranges, full-ROM coverage, zero gaps/overlaps, and SOURCE_OWNED 1,487,672 (delta 0). Add focused regression tests, preserve M14.2A/B/C acceptance, run Debug/Release full CTest, file-limit and diff checks, and fresh-worktree smoke before publication.

**Initial evidence review:** Existing `knowledge.sqlite` already holds 1,926 static instruction objects, 2,651 scoped runtime instruction occurrences, 2,249 observed runtime relations, 1,007 Carver hypotheses, and 38 exact DMA/SAT relations. Runtime succession is not a closed CFG proof. Exact-object overlap includes alternate decodes sharing wider ROM ranges. No canonical ROM byte file is available in this checkout for decoding and assembler roundtrip, so no classification or promotion may be inferred from these signals alone. The campaign must surface this blocker and the highest-impact generic improvement rather than invent a map delta.

**Implementation and campaign result:** Added `rom_unknown_range_campaign.py`, a deterministic ranker over all canonical UNKNOWN emissions using existing objects, claims, relations, evidence refs, adjacent emission classes, and hypothesis labels. It emits blocker fingerprints and does not project map operations from rankings. The preserved M14.2C child ranked all 768 intervals / 1,658,056 bytes; all-range ranking hash is `af04d666b708e4d085237a04c22a76ccff7c708818874eb10ae3dda4b449f092`. Top candidate `[0x061588,0x061CD2)` has 177 executed PCs, 16 incoming references, 218 graph relations, and four independent artifact hashes. Exact instruction objects cover 650 of its 1,866 bytes, leaving gaps; observed succession is not closed CFG proof. The `decoder_eos` label remains a hypothesis. No canonical byte artifact was supplied in this evidence generation, so decode and assembler roundtrip could not be established. The campaign correctly stops with zero classified bytes and zero proposals.

**Acceptance metrics:** `STOP_NO_EXACT_CLASSIFICATION`; map hash `d520a6da1be5d730c4352eacc75ce70fd47a23465d6899282cea7188ad416f88`; graph hash `3d0b19f11a1d8fa03a7c68a98756ef6c87f345a4aa74781166a9900bebe06d78`; emission hash `44a2332b0b433c635e33767886ffff35985ad31131e3dc3b4dea5e6984b17d92`. Full ROM 3,145,728 bytes; 2,489 ranges; zero gaps/overlaps. UNKNOWN remains 768 ranges / 1,658,056 bytes. New classified bytes/ranges 0; new boundaries/references/symbols 0. SOURCE_OWNED is 1,487,672 before and after (delta 0). Blocker groups: 27 no closed CFG/byte roundtrip, 302 hypothesis-only boundary/format, and 439 no exact consumer/extent. The largest generic improvement is a generic canonical-byte decoder plus closed-CFG/assembler-roundtrip gate. Report and receipt: `docs/reports/THOR_M14_3_UNKNOWN_ROM_RANGE_CLOSURE_CAMPAIGN.md` and `.json`.

**Validation:** M14.2A acceptance 3/3; M14.2B fresh two-capture acceptance PASS and order-independent graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`; M14.2C closure PASS. Fusion (4), canonical map (2), closure (3), MASTER V2 (3), canonical materialization (3), and campaign (2) focused tests pass. Debug build and full CTest pass (211/211); Release build and full CTest pass (211/211). Deterministic campaign receipt replay matches. Source-code limit passes (739 governed files at or below 500 lines); `git diff --check` passes. Fresh-worktree smoke remains the final publication check.

**Fresh-worktree smoke:** Detached smoke checkout replayed M14.2B from the accepted canonical parent and two captures; fusion acceptance PASS, forward/reverse graph hash `598c42312e80e1d87a871bca75412dfa10f0d7a2eb0d5e812fc2d8a2e3f29b65`. M14.2C closure PASS with 2,489 ranges and zero gaps/overlaps. M14.3 campaign reranked all 768 intervals; ranking hash matched the tracked receipt, and SOURCE_OWNED remained 1,487,672. M14.2A/B/C, map, MASTER V2, materialization, and campaign focused tests passed in the detached worktree; `git status` was clean.

# M14.4 — Canonical ROM generic ASM closure

**Objective:** Verify the supplied canonical ROM identity, reuse the accepted M68K decoder and canonical SQLite evidence/map generations, and attempt generic exact-seed CFG closure plus assembler roundtrip over the 27 M14.3 code-blocked UNKNOWN intervals. No address-specific decode behavior, map classification without proof, or SOURCE_OWNED changes.

**Acceptance:** SHA-bound ROM reads; exact persisted seed provenance; recursive CFG with explicit unresolved blockers; instruction-boundary collision checks; exact extents and assembler roundtrip before any parent-bound map proposal; deterministic report over the primary fixture and all 27 blocked intervals. Preserve canonical partition and SOURCE_OWNED 1,487,672. Run focused regressions, M14.2A/B/C and M14.3 acceptance, MASTER V2/materialization, Debug/Release full CTest, source limit, diff check, and fresh-worktree smoke before publishing.

**Initial architecture finding:** Existing `decode_m68k_slice` explores only addresses at or above its entry, while `slice_asm` requires a fully contiguous decoded slice. A valid generic solution must merge independently proven exact-seed decodes, validate their combined graph and boundaries, then emit only separate contiguous extents. The UNKNOWN interval cannot be passed to the contiguous emitter as a code extent.

**Implementation:** Added the M14.4 verified bounded ROM provider, a reusable CFG-closure wrapper over the accepted decoder, a CLI that emits separate contiguous extents, and a SQLite-backed campaign driver. Exact runtime/reconstruction claims and typed exact canonical-reference/ASM-CFG relation targets seed closure. Seeds merge by stable ROM PC/object identity while capture claims, relations, and source evidence references stay attached. Exact external edges require an existing canonical ASM instruction start. Only fully closed candidates whose every contiguous extent roundtrips produce parent-bound `SPLIT_RANGE`, `ADD_BOUNDARY`, `CLASSIFY_RANGE(ASM)`, and exact decoded `ADD_REFERENCE` proposal operations; proposals live in a copied SQLite generation and do not alter accepted map emissions.

**Campaign result:** Canonical ROM size/hash verified. The primary `[0x061588,0x061CD2)` interval retained its 1,866-byte UNKNOWN envelope: 177 runtime seeds yielded 241 decoded instructions / 926 bytes, but closure stopped at `CFG_ESCAPES_UNKNOWN_WITHOUT_EXACT_TARGET` and assembly returned `ASSEMBLER_ERROR`. All 27 blocked intervals were analyzed; five CFG components closed and their five exact extents roundtripped for 260 bytes. They produce 31 parent-bound operations (5 splits, 10 boundaries, 5 classifications, 11 direct references; proposal id `m14-4-proposal:8073082b1d516c4bb8ac47685ba605a5694bb9565ff5f45c1b4a2016445cd6f9`, proposal hash `f9b2965a000c26d47203a49eef738e0f6eb013f7904f545e4a8cb4d8c0714af9`) in an ignored proposal-only SQLite copy. No operations were applied. Full campaign replay matched exactly. Canonical map remains 2,489 ranges / 3,145,728 bytes, zero gaps/overlaps, 768 UNKNOWN ranges / 1,658,056 bytes; SOURCE_OWNED remains 1,487,672 (delta 0).

**Validation:** M14.2A 3/3, fusion 4/4, M14.2C closure 3/3, M14.3 2/2, MASTER V2 3/3, canonical materialization 3/3, knowledge map 2/2, knowledge pipeline 18/18, and both M14.4 CTest entries pass. Debug build and full CTest pass (213/213); Release build and full CTest pass (213/213). The canonical-byte campaign and deterministic replay pass; canonical partition is 2,489 ranges over 3,145,728 bytes with zero gaps/overlaps, and SOURCE_OWNED remains 1,487,672 (delta 0). Fresh detached worktree at `73c96d7018197315bd0f89abdb70585f44a7e7cc` rebuilt the CLI, passed M14.4 closure/campaign and source-limit tests (3/3), and reproduced the proposal hash `f9b2965a000c26d47203a49eef738e0f6eb013f7904f545e4a8cb4d8c0714af9`; its source tree was clean. Final diff review and publication remain.

# M14.5 — Exact ASM map adoption and Stage7 eligibility

**Objective:** Independently reproduce and validate the published M14.4 31-operation proposal, apply only exact non-ownership map operations to deterministic child generations, preserve historical evidence and range lineage, reconcile the global graph with new canonical ASM extents, and audit each component against existing Stage7 promotion gates. Do not discover new ranges, infer gameplay meaning, bypass Stage7, or change SOURCE_OWNED except through its established promotion path.

**Acceptance:** Bind proposal application to the exact canonical ROM, generation, map, graph, and emission hashes; recheck CFG, instruction boundaries, canonical bytes, exact assembler roundtrip, and references; reject stale parents; apply accepted operations independently; compare two same-parent application results including identities, lineage, hashes, and partition; verify global-object projection; report per-component Stage7 eligibility and blockers. Preserve full-ROM coverage and the 1,487,672-byte SOURCE_OWNED baseline unless the complete existing promotion contract is satisfied. Run M14.2A/B/C, M14.3/M14.4, map/split/stale-parent/reconciliation/Stage7/Master V2 tests, full Debug and Release CTest, source-size and diff checks, and fresh-worktree smoke before publication.

**Initial state:** New branch `codex/m14-5-exact-asm-map-adoption` starts at the clean published M14.4 base `29aa26973ca146140b40236b042526755b9649e1`. Existing map proposal support persists parent-bound operations and checks stale parents, but does not apply split/classification operations. The canonical emission table has no split-lineage table; preserve history through the existing derivation and proposal-operation records rather than destructively rewriting graph witnesses. The existing source-owned promotion tooling requires independent static caller/entry evidence and whole-ROM materialization in addition to exact slice assembly; M14.4 roundtrip alone does not satisfy that contract.

**Result:** `PASS_EXACT_ASM_MAP_ADOPTION_V1`. The tracked 31-operation proposal reproduced with the expected M14.4 ID/hash and base generation/map/graph/emission identities. Independent validation applied all five exact components (260 bytes) and rejected six unsupported `ADD_REFERENCE` operations independently; the remaining 25 operations were accepted. Two applications generated identical child map/emission/generation hashes, range identities, classifications, references and lineage. The map now has 2,490 emission rows, 764 UNKNOWN ranges / 1,657,796 UNKNOWN bytes, and zero gaps/overlaps. `SOURCE_OWNED` remains 1,487,672 (delta 0). The original `[0x061588,0x061CD2)` UNKNOWN interval remains unresolved. All five global object views include canonical ASM classification, M14.4 proof, runtime occurrence, CFG relations and adoption derivation. Stage7 eligibility is zero: static caller support and whole-ROM reconstruction proof are absent, so no promotion ran.

**Validation:** M14.2A acceptance 3/3; M14.2B fusion, M14.2C closure, M14.3 campaign and M14.4 closure focused tests pass. M14.5 contract test passes (2/2). MASTER V2 3/3 and canonical materialization 3/3 pass. Full Debug build/CTest pass (214/214); full Release build/CTest pass (214/214). `project_file_line_limit` passes in both. `git diff --check` passes. Fresh detached worktree `3f3f14ad4dbd0dc5bcf63e4fec9b866f36d21fcd` configured, rebuilt `oasis_re_m14_4_cfg`, and passed M14.4/M14.5/source-limit smoke (3/3). Branch `codex/m14-5-exact-asm-map-adoption` was pushed to origin and the remote head was verified.

# M14.6 — Exact ASM source-ownership closure

**Objective:** Audit all five non-owning M14.5 ASM components against the actual Stage7 admission and reconstruction path, close only generic integration blockers backed by already accepted M14.4/M14.5 exact evidence, and promote only through the existing Stage7 transaction. Do not invent static callers, reconstruct missing parent source artifacts from ROM bytes, weaken any promotion gate, or change ownership directly.

**Acceptance:** Verify exact map rows, CFG/roundtrip proofs, source form and fresh vasm bytes for all five extents; call the existing `re_m12_auto_promote.selected` selector using canonical verified caller relations; evaluate the existing UNKNOWN-only split/materialization contract; classify evidence/capability/integration/contradiction blockers; reconcile any exact CFG operations rejected only because their source instruction object was introduced by M14.5 in the child; deterministically replay the graph child; preserve all M14.5 classifications and `SOURCE_OWNED` unless existing Stage7 completes. Run focused and full Debug/Release validation, line limit, diff check, and fresh-worktree smoke before publication.

**Initial state:** Clean branch `codex/m14-6-exact-asm-source-ownership-closure` starts at published M14.5 head `c2e9aaaa4caf04909dfc8a92877ac3426aa333e8`. The M14.5 child SQLite is available locally under ignored `build/thor-evidence/`. Direct execution of the existing Stage7 selector against canonical static-caller counts excludes all five components (`called_by=0`). Six parent-bound M14.4 `ADD_REFERENCE` operations were rejected in M14.5 because their source instruction objects did not exist in the old parent, although exact M14.5 instruction objects now exist in the adopted child. Reconcile those operations only from their existing exact proof refs; they do not constitute caller evidence or ownership proof.

**Result:** `PASS_EXACT_ASM_SOURCE_OWNERSHIP_CLOSURE_V1`. All five adopted components (260 bytes) retain `ASM_ROUNDTRIP_EXACT`, non-owning map truth, closed CFG, canonical bytes, available source forms and exact fresh vasm roundtrips. The actual Stage7 selector admits zero; no promotion was attempted. The selected generic fix closes M14.5's proof-bound CFG reference admission gap for two components / 158 bytes: all 11 M14.4 reference operations now exist in the canonical graph (six inserted, five replayed). Two same-parent child applications match in every audited logical row and stable generation/map/graph/emission hash. The resulting map remains 2,490 ranges over 3,145,728 bytes, 1,657,796 UNKNOWN bytes, zero gaps/overlaps; `SOURCE_OWNED=1,487,672`, delta 0. Unresolved ranked gates: no static caller/entry evidence (EVIDENCE_GAP, 5 components / 260 bytes); Stage7 reconstruction manifest and parent source artifacts unavailable (CAPABILITY_GAP, 5 / 260); and the Stage7 UNKNOWN-only split cannot ingest already adopted ASM (INTEGRATION_GAP, 5 / 260). The next generic capability is restoring the accepted Stage7 full-ROM manifest and parent artifact set; exact caller evidence must still be provided before any promotion.

**Validation:** M14.2A 3/3; M14.2B fusion 4/4; M14.2C closure 3/3; M14.3 2/2; M14.4 closure 2/2; M14.5 2/2; M14.6 Stage7 contract 4/4; knowledge pipeline 18/18; MASTER V2 3/3; canonical materialization 3/3. Debug build/full CTest PASS (215/215); Release build/full CTest PASS (215/215), including `project_file_line_limit` in both. The accepted-ROM exact source/roundtrip campaign and deterministic two-child replay pass. Fresh detached worktree `aefb4d0` configured successfully, built `oasis_re_cfg_closure_test`, passed the M14.6 contract (4/4) and source-limit CTest (2/2). `git diff --check` passes. Branch `codex/m14-6-exact-asm-source-ownership-closure` was pushed to origin and its remote head was verified.

# M14.7 — Static entry proof and Stage7 promotion readiness

**Objective:** Audit the five M14.5 exact non-owning ASM components for exact static control-flow entries in canonical SOURCE_OWNED 68K ASM, using the accepted decoder and existing SQLite graph. Admit only proof-complete STATIC_VERIFIED caller/entry facts; replay the real Stage7 selector; regenerate full-ROM promotion artifacts only if a component passes that selector. Do not infer entries from runtime, weaken Stage7, or touch neighboring UNKNOWN bytes.

**Acceptance:** Enumerate all five component graph neighborhoods and runtime observations; scan only canonical verified source-owned ASM at accepted boundaries; decode candidate instruction spans from canonical bytes; validate exact direct-call target entry boundaries; admit deterministic claims/relations and provenance only when the current Stage7 contract accepts the evidence; otherwise report the exact generic blocker and leave truth unchanged. Preserve 2,490 map ranges, 764 UNKNOWN ranges / 1,657,796 bytes, 1,487,672 SOURCE_OWNED unless normal Stage7 promotion completes. Require deterministic replay and the full Debug/Release, focused milestone, line-limit, diff, and fresh-worktree validations before publishing.

**Initial state:** Clean branch `codex/m14-7-static-entry-stage7-readiness` starts at the exact requested remote head `ea3b936cf457ca1b91d1ee6bb3fabfd5c50c338c`. The accepted M14.6 generation `gen-m14-6-dcb2596abccf9612` is available locally in ignored build evidence. The actual selector admits exact code only when `called_by > 0`; caller support is counted from incoming `DIRECT_CALL_TARGET` or `STATIC_CALLER` relations with `STATIC_VERIFIED` status. The five entries have no such relations. A bounded initial census found 1,149 existing M68K instruction objects inside source-owned ASM; a complete scan must also use the validated source-owned 68K ASM intervals without decoding header vectors, data or Z80 code as 68000.
