# M12-AUTO3 — bounded provenance toward a 90% source-owned ROM map

Status: `M12_AUTO3_BLOCKED_BELOW_90_NO_COMPLETE_PROVENANCE_GRAPH`.
...
| Final local transaction | `build/m12-auto3-script-transaction-b/materialized/manifest.json` |
...
The 90% threshold is 2,831,156 bytes; the current gap is 2,101,498 bytes.
...
| Confirmed structured data | 25,416 | 0.807952881% |
| Confirmed alignment padding | 47 | 0.001494090% |
| Local ROM-derived assets | 654,777 | 20.814800262% |
| **SOURCE_OWNED** | **729,658** | **23.195203145%** |
| Remaining `UNKNOWN` blob | 2,416,070 | 76.804796855% |
...
bytes. The Z80 8192-byte image is included in the ASM total and is also
...
10. Indexed script table `[0x51514,0x515D8)` and 98 NUL-terminated streams
    `[0x515FC,0x541FD)`.
11. Remaining ROM spans retained as canonical-local-ROM blobs.
...
| `0x0541FD..0x060000` | 48,643 | mixed static code/data area with unresolved boundaries |
...
repository. Debug MinGW and Release MinGW full CTest each passed 84/84 after
the indexed-script helper was added; the GNU/Linux-equivalent build and four
relevant M12 helper tests passed. `git diff --check`, source-file limits,
Python helper checks, and the exact manifest/hash audit passed.
