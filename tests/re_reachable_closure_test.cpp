...
void test_merge_and_boundaries() {
    ...
    const auto unsupported = analyze_bounded_backward_register(linear({instruction(0x1000, 0x205F, {0x20, 0x5F}, "movea"),
        instruction(0x1002, 0x4A28, {0x4A, 0x28, 0x00, 0x01})}), 0x1002, 0);
    assert(unsupported.reason == ClosureReason::unsupported_transfer);
