...
int main(int argc, char** argv) {
    ...
    try {
        ...
        expect_bytes(rom, 0x17CA6,
                     {0x08, 0x2E, 0x00, 0x07, 0x00, 0x4C, 0x67, 0x14,
                      0x08, 0xEE, 0x00, 0x01, 0x00, 0x36, 0x4A, 0x39,
                      0x00, 0xFF, 0x19, 0x85, 0x67, 0x06, 0x08, 0xAE,
                      0x00, 0x01, 0x00, 0x36, 0x08, 0x39, 0x00, 0x01,
                      0x00, 0xFF, 0x0D, 0xBA, 0x67, 0xD8, 0x10, 0x3C,
                      0x00, 0x02, 0x72, 0xF6, 0x74, 0xFA, 0x76, 0x0A},
                     "observed target query setup mismatch");
        expect_bytes(rom, 0x17CDC,
                     {0x4E, 0xB9, 0x00, 0x00, 0xB9, 0x22, 0x65, 0xC0,
                      0x0C, 0x68, 0x00, 0x16},
                     "observed target query call mismatch");
        expect_bytes(rom, 0xB922,
                     {0x48, 0x80, 0x48, 0xC0, 0x3E, 0x2E, 0x00, 0x08,
                      0xD2, 0x47, 0xD6, 0x47, 0x3E, 0x2E, 0x00, 0x0C,
                      0xDE, 0x6E, 0x00, 0x14, 0xD4, 0x47, 0xD8, 0x47,
                      0x3E, 0x2E, 0x00, 0x10, 0xDE, 0x6E, 0x00, 0x14},
                     "observed target scan mismatch");
        // 0xFFDE allocates from FF2D8C, loads a data-driven type and installs
        // callback 0x17A96; 0x17BA4 then enters the FF19E8 query path.
        expect_bytes(rom, 0xFFDE,
                     {0x61, 0x00, 0xDA, 0x10, 0x65, 0x00, 0xF9, 0x38,
                      0x52, 0x88, 0x1A, 0x18, 0x3D, 0x58, 0x00, 0x00,
                      0x4E, 0xB9, 0x00, 0x00, 0x8D, 0x06, 0x1D, 0x45,
                      0x00, 0x4C, 0x2D, 0x7C, 0x00, 0x01, 0x7A, 0x96},
                     "observed cross-pool initializer mismatch");
        expect_bytes(rom, 0xD9F0,
                     {0x48, 0xE7, 0x40, 0x00, 0x4D, 0xF9, 0x00, 0xFF,
                      0x2D, 0x8C},
                     "observed owner pool allocator mismatch");
        expect_bytes(rom, 0x17BA4,
